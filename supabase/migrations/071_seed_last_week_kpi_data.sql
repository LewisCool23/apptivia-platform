-- Migration 071: Seed KPI data for last week (March 9–15, 2026)
-- Adds one week of data for all reps across all 21 active KPI metrics.
-- Uses the same tiered multiplier approach as migration 061.
...
SELECT
  ...
  GREATEST(0, ROUND(
    ...
    END
  ::numeric, 1)) AS value,
  '2026-03-09'::date AS period_start,
  '2026-03-15'::date AS period_end
FROM kpi_metrics km
...
WHERE km.is_active = true
  AND NOT EXISTS (
    ...
    WHERE kv.kpi_id = km.id
      ...
      AND kv.period_start = '2026-03-09'::date
      AND kv.period_end = '2026-03-15'::date
  );
