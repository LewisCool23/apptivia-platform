      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl mx-4 mb-10 review-modal-content">
        ...
        <div className="p-6 border-b flex items-start justify-between print:border-b-2">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600">Rep: {repLabel}</p>
        ...
        <div className="p-6 space-y-6">
          ...
          {(review.final_rating || review.manager_rating) && (
            <div className="flex items-center gap-4 bg-gray-50 rounded-lg p-4">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(i => (
                  <Star key={i} size={24} className={i <= (review.final_rating || review.manager_rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'} />
                ))}
              </div>
              {avgScore != null && (
                <div className="ml-auto text-center">
                  <div className="text-2xl font-bold text-blue-600">{avgScore}%</div>
                  <div className="text-xs text-gray-500">Avg. Scorecard</div>
                </div>
              )}
            </div>
          )}
          ...
          {review.kpi_attainment?.length > 0 && (
            <section>
              <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-1.5"><Target size={14} /> KPI Attainment</h3>
                  ...
                  <tbody>
                    {review.kpi_attainment.map((kpi, idx) => (
                      <tr key={idx} className="border-b border-gray-100">
                        ...
                        <td className="py-2 text-gray-500">{kpi.weeks_measured}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {review.achievements_earned?.length > 0 && (
            <section>
              <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-1.5"><Award size={14} /> Achievements Earned ({review.achievements_earned.length})</h3>
                ...
                {review.achievements_earned.map((a, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-yellow-50 rounded-lg px-3 py-2">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-gray-500">{a.category} · {a.tier}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="bg-blue-50 rounded-lg p-4 space-y-3">
                ...
                {review.rep_accomplishments?.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Key Accomplishments</div>
                ...
                {review.rep_challenges?.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Challenges</div>
                ...
                {review.rep_goals_next_period?.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Goals for Next Period</div>
                    <ul className="space-y-1">{review.rep_goals_next_period.map((g, i) => (
                      <li key={i} className="text-sm text-gray-700">→ {g.text}</li>
                    ))}</ul>
                  </div>
                )}
                {review.rep_comments && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Comments</div>
                    <p className="text-sm text-gray-700">{review.rep_comments}</p>
                  </div>
                )}
              </div>
            </section>
          ...
          {isManager && (review.status === 'manager_review' || review.status === 'reopened') && (
            <section className="print-hidden">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-900">Manager Assessment</h3>
                {onGenerateAiDraft && (
                  <button
                    type="button"
                    onClick={async () => {
              ...
              <div className="space-y-4 bg-purple-50 rounded-lg p-4 border border-purple-100 relative">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Overall Summary</label>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold text-gray-700">Strengths</label>
                    <button type="button" onClick={() => addItem(setMgrStrengths)} className="text-xs text-blue-600"><Plus size={12} /> Add</button>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold text-gray-700">Areas for Improvement</label>
                    <button type="button" onClick={() => addItem(setMgrImprovements)} className="text-xs text-blue-600"><Plus size={12} /> Add</button>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button type="button" onClick={() => addItem(setMgrGoals)} className="text-xs text-blue-600"><Plus size={12} /> Add</button>
                  </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Manager Rating</label>
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map(i => (
                      <button key={i} type="button" onClick={() => setMgrRating(i)}>
                        <Star size={24} className={i <= mgrRating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 hover:text-yellow-200'} />
                      </button>
                    ))}
                    {mgrRating > 0 && <span className="text-sm text-gray-600 ml-2">{mgrRating}/5</span>}
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Final Rating</label>
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map(i => (
                      <button key={i} type="button" onClick={() => setFinalRating(i)}>
                        <Star size={24} className={i <= finalRating ? 'text-emerald-400 fill-emerald-400' : 'text-gray-200 hover:text-emerald-200'} />
                      </button>
                    ))}
                    {finalRating > 0 && <span className="text-sm text-gray-600 ml-2">{finalRating}/5</span>}
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Final Summary</label>
                ...
                {review.manager_strengths?.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1">{review.manager_strengths.map((s, i) => (
                      <li key={i} className="text-sm text-gray-700 flex items-start gap-1.5"><CheckCircle2 size={12} className="text-green-500 mt-0.5 shrink-0" /> {s.text}</li>
                    ))}</ul>
                  </div>
                )}
                {review.manager_areas_for_improvement?.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Areas for Improvement</div>
                    <ul className="space-y-1">{review.manager_areas_for_improvement.map((a, i) => (
                      <li key={i} className="text-sm text-gray-700">• {a.text}</li>
                    ))}</ul>
                  </div>
                )}
                {review.final_summary && (
                  <div>
                    <div className="text-xs font-semibold text-gray-700 mb-1">Final Summary</div>
                    <p className="text-sm text-gray-700">{review.final_summary}</p>
                  </div>
                )}
              </div>
            </section>
