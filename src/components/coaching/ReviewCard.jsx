    ...
    <div className={`border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow flex flex-col border-l-4 ${cfg.borderColor}`}>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h4 className="font-semibold text-apptivia-ink mb-1">{review.title}</h4>
          <p className="text-xs text-apptivia-carbon-500">{repLabel}</p>
        </div>
      ...
      {ratingStars && (
        <div className="flex items-center gap-1 mb-2">
          {[1, 2, 3, 4, 5].map(i => (
            <Star key={i} size={14} className={i <= ratingStars ? 'text-yellow-400 fill-yellow-400' : 'text-apptivia-carbon-300'} />
          ))}
          <span className="text-xs text-apptivia-carbon-500 ml-1">{ratingStars}/5</span>
        </div>
      )}
