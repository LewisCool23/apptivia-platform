...
export default function ReviewTab({ teamMembers, startForRepId }) {
  ...
  return (
    <div>
        ...
        <div className="flex gap-1.5">
          {[
            ...
            <button key={tab.id} onClick={() => setTypeFilter(tab.id)}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                typeFilter === tab.id ? 'bg-apptivia-ink text-white' : 'bg-apptivia-carbon-100 text-apptivia-carbon-600 hover:bg-apptivia-carbon-200'
              }`}>
              {tab.label}
            </button>
          ))}
      ...
      {/* Cards */}
      {loading ? (
        <div className="text-center py-12 text-sm text-apptivia-carbon-500">Loading reviews...</div>
      ) : filtered.length === 0 ? (
