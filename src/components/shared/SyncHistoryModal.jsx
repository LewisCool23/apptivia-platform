...
const STATUS_BADGE = {
  ...
  skipped:    'bg-apptivia-carbon-100 text-apptivia-carbon-500',
};
  ...
  return (
    <div className="mt-4 border-t pt-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-apptivia-ink">CRM Push</h3>
        <div className="flex items-center gap-2">
          <button onClick={() => loadData('both')} className="text-apptivia-carbon-400 hover:text-apptivia-carbon-600" title="Refresh">
            <RefreshCw size={13} />
          </button>
          <button
            onClick={handlePushNow}
            disabled={pushing}
            className="inline-flex items-center gap-1 text-xs font-medium text-apptivia-ink hover:text-apptivia-ink disabled:opacity-50"
          >
            <Send size={11} />
        ...
        {['queue', 'history'].map(t => (
          <button key={t} onClick={() => { setTab(t); loadData(t); }}
            className={`px-2.5 py-1 text-xs rounded-full font-medium transition-colors ${
              tab === t ? 'bg-apptivia-ink text-white' : 'bg-apptivia-carbon-100 text-apptivia-carbon-600 hover:bg-apptivia-carbon-200'
            }`}>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-4 text-apptivia-carbon-400 text-xs">
          <Loader2 size={14} className="animate-spin mr-1.5" /> Loading...
        </div>
      ) : tab === 'queue' ? (
        queue.length === 0 ? (
          <p className="text-xs text-apptivia-carbon-400 text-center py-3">Queue is empty</p>
        ) : (
        ...
        pushHistory.length === 0 ? (
          <p className="text-xs text-apptivia-carbon-400 text-center py-3">No push history yet</p>
        ) : (
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {pushHistory.slice(0, 10).map(entry => (
              <div key={entry.id} className="flex items-center justify-between py-1.5 text-xs border-b border-apptivia-carbon-100">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${STATUS_BADGE[entry.status] || 'bg-apptivia-carbon-100 text-apptivia-carbon-500'}`}>
                    {entry.status}
                  </span>
                  <span className="text-apptivia-carbon-400 text-[10px]">
      ...
      <div className="bg-white w-[95%] max-w-2xl rounded-lg shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-apptivia-carbon-500">Recent synchronization runs</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-sm text-apptivia-carbon-500 text-center py-8">No sync history yet.</div>
        ) : (
              ...
              <div key={entry.id} className="border rounded-lg p-3 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {entry.status === 'running' && <Loader2 size={14} className="text-apptivia-coral animate-spin" />}
                ...
                <div className="text-right text-xs text-apptivia-carbon-500">
